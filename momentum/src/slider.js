...
async function getUnsplashImage() {
    ...
    //const id = 'NwUcAo05yZVNd6d1s-DCaeliR98PI-o8iVXCymFjZ8o';
    ...
    showSlider();
        ...
        ShowHideModal(true, `${state.language === 'ru' ? 'Unsplash API: Ваш лимит 50 фотографий в час исчерпан. Попробуйте позже.' : 'Unsplash API: Your limit of 50 photos per hour has been reached. Try later.'} `);
        urlImg = '';
    }
...
async function getFlickrImage() {
    ...
    showSlider();
        ...
        document.querySelector('body').removeAttribute('style');
        ShowHideModal(true, `${state.language === 'ru' ? 'Flickr API. Ошибка сервера. Попробуйте позже.' : 'Unsplash API: Server error. Try later.'} `);
        urlImg = '';
    }
